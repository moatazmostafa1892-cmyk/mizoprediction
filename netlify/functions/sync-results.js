...
async function syncResults() {
  const now = new Date();
  if(now < new Date("2026-06-11") || now > new Date("2026-07-20")) return {statusCode:200,body:"Outside tournament"};

  const r = await fetch("https://api.football-data.org/v4/competitions/2000/matches?status=FINISHED",{headers:{"X-Auth-Token":FOOTBALL_DATA_KEY}});
  ...
  if(changed>0){
    await fetch(`${FIREBASE_URL}/actuals.json`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(existing)});
  }
  return {statusCode:200,body:JSON.stringify({ok:true,changed})};
}

export const handler = schedule("*/5 * * * *", syncResults);
