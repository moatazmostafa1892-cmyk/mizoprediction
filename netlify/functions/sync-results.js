// Netlify Function — fetches WC2026 results server-side (no CORS issues)
// Uses worldcup26.ir directly — no API key needed, free, reliable
...
const NORM = {
  ...
  "Curaçao":"Curaçao","Curacao":"Curaçao","Côte D'Ivoire":"Ivory Coast",
};
...
exports.handler = async () => {
  ...
  try {
    // Fetch from worldcup26.ir — free, no key, works server-side
    ...
    for(const g of games) {
      ...
      if(ex && ex.source === 'manual') continue; // never overwrite manual
      if(!ex || ex.hg !== hg || ex.ag !== ag) {
        updates[mid] = { hg, ag, source: 'api', ts: Date.now() };
      }
    }

    // Write each result individually to Firebase
